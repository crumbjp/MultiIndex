...
/*
public abstract class BkNonUniqueIndex<K extends Comparable<K>,T> extends UnmodifiableMultiMapWrapper<K,Container<T>> implements Index<T> , MultiMap<K,Container<T>>{
	private Field field;
	protected abstract MultiMap<K,Container<T>> createContainer(final List<Container<T>> origin,final Integer size); 
	public BkNonUniqueIndex(final Field field) {
		this.field = field;
	}
	@Override

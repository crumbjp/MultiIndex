...
public class AvlTree<T,K> {
	final AvlIterator<T,K> itend = new AvlIterator<T,K>(this);
	AvlNode<T> avl = null;
	...
	public AvlIterator<T,K> begin() {
		...
		return  new AvlIterator<T,K>(this,avl.begin());
	}
	...
	public AvlIterator<T,K> last() {
		...
		return  new AvlIterator<T,K>(this,avl.last());
	}
	...
	public boolean erase(AvlIterator<T,K> it) {
		if (it.tree != this ) {
			if ( it.isEnd() ) 
				return false;
	...
	public boolean rawReplace(AvlIterator<T,K> it , T t) {
		if (it.tree != this ) {
			throw new RuntimeException("Failure : Othe tree iterator !");
		}
...
//*
	...
	public AvlIterator<T,K> find(K k,FindComparator<T, K> findComparator) {
		...
		while (true) {
			...
			if ( cmp > 0 ) {
				...
				return new AvlIterator<T,K>(this,curNode);
			}
		}
	}
	...
	public AvlIterator<T,K> findFirst(K k,FindComparator<T, K> findComparator) {
		...
		return new AvlIterator<T,K>(this,retNode);
	}
	...
	public Pair<AvlIterator<T,K>,AvlIterator<T,K>> equalRange(K k,FindComparator<T, K> findComparator) {
		...
		return new Pair<AvlIterator<T,K>, AvlIterator<T,K>>(
				new AvlIterator<T,K>(this,firstNode),
